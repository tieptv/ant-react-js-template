...
const InfoView = ({ title = "", data, rowNumber = 1, nonBorder = false }) => {
      ...
      <div className={`grid grid-cols-1 ${gridCols} gap-2`}>
        {data.map((item, index) => (
          <div key={index} className="flex">
            <div className="font-normal text-xs w-40 flex-1">{item.label}</div>
            <div className="font-bold text-xs flex-1">{item.value}</div>
          </div>
        ))}
      </div>
    </div>
